...
- If line_edit reports that the file was not read first, call file_read on that file and then retry with line numbers from the latest output.
...
- LINE NUMBERS COME FROM file_read: line_edit only works with the latest file_read output for that file. Use the 1-based line numbers shown there, not guessed numbers.
...
- For large edits, prefer file_write to rewrite the whole file rather than making many small file_editor or line_edit calls.
